...
def _make_sparse_normal(n_features: int, rng: np.random.Generator, feature_names: list[str]) -> np.ndarray:
    ...
    chosen = rng.choice(benign_indices, size=min(n_features, len(benign_indices)), replace=False)
    vec[chosen] = 1.0
    return vec


def _make_sparse_failure(n_features: int, rng: np.random.Generator, feature_names: list[str]) -> np.ndarray:
    ...
    attack_indices = [
        feature_names.index(f) for f in [
            ...
            "sshd+failed password invalid user", "sshd+disconnecting authenticating too many",
            "directory traversal+passwd", "command injection+exec",
            "429+rate limit exceeded", "login failed+401 unauthorized",
        ] if f in feature_names
    ]
    chosen = rng.choice(attack_indices, size=min(n_features, len(attack_indices)), replace=False)
    vec[chosen] = 1.0
    return vec


def _make_dense_normal(rng: np.random.Generator, feature_names: list[str]) -> np.ndarray:
    """Generate a dense normal-event vector (3-5 features, simulating synthetic training data)."""
    return _make_sparse_normal(rng.integers(3, 6), rng, feature_names)


def _make_dense_failure(rng: np.random.Generator, feature_names: list[str]) -> np.ndarray:
    """Generate a dense failure-event vector (3-7 features, simulating synthetic training data)."""
    return _make_sparse_failure(rng.integers(3, 8), rng, feature_names)


def generate_dataset(n_normal: int = 12000, n_failure: int = 6000) -> tuple[np.ndarray, np.ndarray]:
...
def train(X: np.ndarray, y: np.ndarray) -> xgb.XGBClassifier:
    ...
    # Check discrimination: all-zeros should NOT be 0.7666
    zeros = scaler.transform(np.zeros((1, X.shape[1]), dtype=np.float32)).astype(np.float32)
    proba_zeros = model.predict_proba(zeros)[0]
    print(f"\nAll-zeros: P(normal)={proba_zeros[0]:.4f} P(failure)={proba_zeros[1]:.4f}")
    ...
    print(f"sshd+failed password only: P(normal)={proba_attack[0]:.4f} P(failure)={proba_attack[1]:.4f}")
    ...
    print(f"systemd+Started only: P(normal)={proba_normal[0]:.4f} P(failure)={proba_normal[1]:.4f}")
...
def export_onnx(model: xgb.XGBClassifier, scaler: SafeMaxAbsScaler) -> None:
    try:
        ...
        (OUTPUT_DIR / "log_classifier_syslog.onnx").write_bytes(onnx_model.SerializeToString())
        print(f"\nExported ONNX to {OUTPUT_DIR / 'log_classifier_syslog.onnx'}")
    except ImportError:
    ...
    if onnx_path.exists():
        ...
        zeros = scaler.transform(np.zeros((1, 100), dtype=np.float32)).astype(np.float32)
        outputs = sess.run(None, {input_name: zeros})
        proba = outputs[1][0]
        print(f"ONNX verification (all-zeros): P(normal)={proba[0]:.4f} P(failure)={proba[1]:.4f}")
        ...
        proba2 = outputs2[1][0]
        print(f"ONNX verification (feature[0]=1): P(normal)={proba2[0]:.4f} P(failure)={proba2[1]:.4f}")
