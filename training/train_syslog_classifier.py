...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--output-dir", type=str, default=str(ROOT / "models" / "syslog")
    )
    return p.parse_args()
...
def train_model(
    ...
    print(
        f"Training: {n_normal} normal, {n_failure} failure,"
    ...
    print(
        f"All-zeros input: P(normal)={zero_proba[0]:.4f},"
        f" P(failure)={zero_proba[1]:.4f}"
    )
...
def main() -> None:
    ...
    df, y, feature_names = generate_syslog_data(
        args.n_normal, args.n_anomaly, args.seed
    )
    ...
    model, scaler, feature_names, metrics = train_model(
        X, y, feature_names, args
    )
