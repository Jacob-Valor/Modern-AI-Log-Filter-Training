...
try:
    import anthropic  # noqa: F401 - imported for availability check
...
class AnthropicConfig:
    ...
    @classmethod
    def from_yaml(cls, path: str | Path) -> "AnthropicConfig":
        with open(path) as f:
...
class AnthropicIntegration:
    def __init__(self, config: AnthropicConfig):
        ...
        self.client = Anthropic(
            api_key=config.api_key, max_retries=config.max_retries, timeout=config.timeout
        )
        self.usage_stats = {
            "total_tokens": 0,
            "input_tokens": 0,
    ...
    def messages(self, messages: list[dict[str, str]], **kwargs) -> dict[str, Any]:
    ...
    def streaming_messages(self, messages: list[dict[str, str]], **kwargs):
        for attempt in range(self.config.max_retries):
            try:
                time.sleep(self.config.rate_limit_delay)
    ...
    def estimate_cost(self, pricing: dict[str, dict[str, float]]) -> dict[str, float]:
        input_cost = (self.usage_stats["input_tokens"] / 1000000) * pricing.get(
            self.config.model, {}
        ).get("input", 3.0)
        output_cost = (self.usage_stats["output_tokens"] / 1000000) * pricing.get(
            self.config.model, {}
        ).get("output", 15.0)
